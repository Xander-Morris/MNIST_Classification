...
def _setup_data():
    ...
    # binary classification (5 or not 5) that I did first off
    """y_train_5 = (y_train == 5) 
    y_test_5 = (y_test == 5)
    sgd_clf = SGDClassifier(random_state=RANDOM_STATE_SEED)
    sgd_clf.fit(X_train, y_train_5)
    # Convert prediction sample into a DataFrame with the correct column names
    sample_df = pd.DataFrame([some_digit], columns=X_train.columns)

    # Predict without warnings
    print(sgd_clf.predict(sample_df))

    return (some_digit, X_train, y_train_5, sgd_clf)"""

    return (X_train, y_train, X_test, y_test, some_digit)

def _plot_precision_recall_vs_threshold(y_train_5, y_scores):
...
def _cross_validate(some_digit, X_train, y_train_5, sgd_clf):
